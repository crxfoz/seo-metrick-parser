...
// NewServer creates a new api s e o parser server but does not configure it
func NewServer(api *operations.SEOParserAPI) *Server {
	s := new(Server)
...
// Server for the s e o parser API
type Server struct {
	...
	SocketPath    flags.Filename `long:"socket-path" description:"the unix socket to listen on" default:"/var/run/s-e-o-parser.sock"`
...
func (s *Server) SetAPI(api *operations.SEOParserAPI) {
	if api == nil {
		s.api = nil
		s.handler = nil
		return
...
func (s *Server) Serve() (err error) {
	...
	if s.hasScheme(schemeUnix) {
		...
		wg.Add(2)
		s.Logf("Serving s e o parser at unix://%s", s.SocketPath)
		go func(l net.Listener) {
			defer wg.Done()
			if err := domainSocket.Serve(l); err != nil {
				s.Fatalf("%v", err)
			}
			s.Logf("Stopped serving s e o parser at unix://%s", s.SocketPath)
		}(s.domainSocketL)
	...
	if s.hasScheme(schemeHTTP) {
		...
		wg.Add(2)
		s.Logf("Serving s e o parser at http://%s", s.httpServerL.Addr())
		go func(l net.Listener) {
			defer wg.Done()
			if err := httpServer.Serve(l); err != nil {
				s.Fatalf("%v", err)
			}
			s.Logf("Stopped serving s e o parser at http://%s", l.Addr())
		}(s.httpServerL)
	...
	if s.hasScheme(schemeHTTPS) {
		...
		wg.Add(2)
		s.Logf("Serving s e o parser at https://%s", s.httpsServerL.Addr())
		go func(l net.Listener) {
			defer wg.Done()
			if err := httpsServer.Serve(l); err != nil {
				s.Fatalf("%v", err)
			}
			s.Logf("Stopped serving s e o parser at https://%s", l.Addr())
